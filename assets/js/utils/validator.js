const Validator = {
  ...
  ratingStars: {
    valid(stars) {
      ...
      return {
        isValid: isValid,
        errors: { format: isValid ? null : 'Stars must be an integer between 0 and 5' },
      };
    },
    isValid(stars) {
  ...
  password: {
    ...
    valid(password) {
      ...
      return {
        isValid: lenOk && ltOk && numOk,
        errors: { length: lenErr, letter: ltErr, number: numErr },
      };
    },
    isValid(password) {
  ...
  url: {
    valid(url) {
      ...
      try {
        ...
        return {
          isValid: isValidUrl,
          errors: { format: isValidUrl ? null : 'Invalid URL format' },
        };
      }
    },
    isValid(url) {
  ...
  imageUrl: {
    valid(imageUrl) {
      ...
      return {
        isValid: urlOk && imgOk,
        errors: { urlFormat: urlErr.format, imageFormat: imgErr.format },
      };
    },
    isValid(imageUrl) {
  ...
  categoryCode: {
    valid(code) {
      ...
      return {
        isValid: isValid,
        errors: { format: isValid ? null : 'Invalid category code format' },
      };
    },
    isValid(code) {
  ...
  recipeCode: {
    valid(code) {
      ...
      return {
        isValid: isValid,
        errors: { format: isValid ? null : 'Invalid recipe code format' },
      };
    },
    isValid(code) {
  ...
  recipeNutrition: {
    valid(nutrition) {
      ...
      if (nutrition.cholesterol !== undefined && nutrition.cholesterol !== null) {
        if (
          typeof nutrition.cholesterol !== 'number' ||
          nutrition.cholesterol < 0 ||
          nutrition.cholesterol > 5000
        ) {
          isValid = false;
  ...
  id: {
    valid(id) {
      ...
      return {
        isValid: isValid,
        errors: { format: isValid ? null : 'ID must be a non-empty string' },
      };
    },
    isValid(id) {
  ...
  string: {
    validLength(str, minLength = 1, maxLength = Infinity) {
      ...
      return [
        isValid ? null : `String length must be between ${minLength} and ${maxLength} characters`,
        isValid,
      ];
    },
  ...
  positiveInteger: {
    valid(num) {
      ...
      return {
        isValid: isValid,
        errors: { format: isValid ? null : 'Must be a positive integer' },
      };
    },
    isValid(num) {
  ...
  nonNegativeInteger: {
    valid(num) {
      ...
      return {
        isValid: isValid,
        errors: { format: isValid ? null : 'Must be a non-negative integer' },
      };
    },
    isValid(num) {
  ...
  roleCode: {
    valid(code) {
      ...
      return {
        isValid: isValid,
        errors: { format: isValid ? null : 'Invalid role code format, must be ROLE_*' },
      };
    },
    isValid(code) {
  ...
  tags: {
    valid(tagsArray) {
      if (!Array.isArray(tagsArray))
        return { isValid: false, errors: { type: 'Tags must be an array' } };
